...
GL/ImGui 版（`magtile_app library --dev-gui`，原 `--gui`，已按 §4 退役条件降级为内部开发工具）证明了核心引擎与 3D 教程闭环可行，但它是**工程验证外壳**，离商用儿童产品有结构性差距：
...
| **QT-6 打包发布** | windeployqt/MSIX、macdeployqt/DMG 公证、Linux AppImage；ImGui 版退役为内部工具 | QT-1~5 | `IN_PROGRESS`（打包脚手架 DONE：install 规则（Qt≥6.5 部署 API 自动收运行库 / 6.4 文档化 windeployqt/macdeployqt）+ CPack 并存/Qt-only 两种包形态（`MAGTILE_PACKAGE_QT_ONLY`）+ starter 30 模型子集叠加 + LGPL 合规清单，操作手册 `scripts/package_qt_desktop.md`；冒烟脚本 DONE：Windows 实机一键 `scripts/smoke_qt_windows.ps1`（环境检测→构建→ctest→CPack NSIS/ZIP→Qt6.4 自动跑 windeployqt / ≥6.5 核对自动部署→包内清单断言（Qt DLL 六件套/qwindows/qml 树/CRT/目录登记一致性）→offscreen 启动→重压 *-deployed.zip，-DryRun 环境报告+断言逻辑自检（含 qwindows/目录缺失双失败注入）已在 pwsh 过）+ Linux 一键 `scripts/smoke_qt_linux_pack.sh` 全绿（三档 TGZ 清单断言 + **NSIS 冒烟升级：cpack -G NSIS 生成 project.nsi 经 makensis 编译通过 + 并存包双快捷方式断言** + starter 解包目录一致性 + offscreen 实测 + ldd 动态链接核验）+ 手册新增第十一节 Windows 实机验收清单（装→启→库→教程→退→卸载 + 缺 VC++ 运行库/QML 插件未拷/qwindows 缺失等排查表）+ macOS 一键 `scripts/smoke_qt_macos.sh`（环境检测→构建→ctest→CPack TGZ→清单断言→offscreen→合成最小 .app + macdeployqt -qmldir→重签（缺省 ad-hoc, --sign-identity 预留）→bundle 断言（Qt 六框架/libqcocoa/Resources/qml 树/bundle 内 data/otool 动态链接, 与 Windows DLL 六件套同口径）→bundle 自足启动→hdiutil DMG（.app + Applications 软链）+ 挂载断言, 非 macOS 上跑可移植子集并对 macOS 专属环节显式 [--] SKIP 标记 PARTIAL, Linux 实跑子集 + --dry-run 双失败注入自检全绿）+ 手册新增第十二节 macOS 实机验收清单（DMG 结构与 bundle 内 data 布局→拖装→Gatekeeper/签名公证放行→启动进教程→退出续档→卸载 + "已损坏"/缺框架/忘 -qmldir 黑屏/arm64 未重签等排查表）；**ImGui 版退役为内部工具 DONE**（`library --gui` → `library --dev-gui`，`--gui` 一期保留别名 + 温和迁移提示，用户文档全面改指 Qt 壳，GL 冒烟照跑，详见下方退役条件段）；windeployqt/macdeployqt 实机运行与干净机验收（跑对应脚本+清单即可）、MSIX、DMG 签名公证、AppImage、许可全文随包 PLANNED） |

**退役条件**：QT-3 完成且教程播放器通过 UI_UX_SPEC §14 验收清单后，`library --gui` 从用户文档移除、降级为 `--dev-gui`（保留冒烟测试）——**已执行（2026-08）**：CLI 图形入口更名为 `library --dev-gui` / `tutorial --dev-gui`（`--gui` 一期保留为别名并打温和迁移提示，后续版本移除），主 README / ROADMAP / 各用户面向文档改为指向 Qt 版 `magtile_studio_qt`，GL 壳定位收敛为内容制作 / 调试 / CI 冒烟内部工具（`gl_smoke` / `library_gui_smoke` / `inventory_gui_smoke` 照跑，含 `--gui` 别名回归检查；ImGui 代码不删除）；并存安装包主快捷方式参数同步为 `library --dev-gui`。QT-6 完成后商店渠道只发 Qt 版（未完成，见 §4 QT-6）。
...
| 进度与成就 §7 | CLI 有数据 | `DONE`（进度页「我的作品」: 三格统计 + 成就墙条带 + 进行中/已完成/收藏列表 + 空态引导；成就墙全览: 未解锁灰色剪影 + 达成条件, 不显示百分比；首页大按钮 + 温和统计卡片双入口）；成就触发定义表 `DONE`（档位定义 + 写库触发统一收口 `progress/achievements.hpp`：GL/Qt/Android 完成链路只调 `unlockAchievementsOnComplete`，按完成数 1/3/10/30 达档幂等写库、老档下次完成自动补录，展示层按完成数兜底点亮口径不变；单测 `achievements_unlock`） | QT-4 |
