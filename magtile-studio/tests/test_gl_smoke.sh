...
if grep -q -- '--gui' <<<"$usage_output"; then
    pass "用法输出包含 --gui 选项"
else
    fail "用法输出未提及 --gui"
fi
...
if [[ "$gui_exit" -eq 0 ]]; then
    ...
    "$APP" tutorial "$MODEL" --data-dir "$DATA_DIR" --gui --frames 1 \
        >/dev/null 2>&1
    link_exit=$?
    if [[ "$link_exit" -eq 2 ]]; then
        fail "构建未包含渲染后端 (--gui 退出码 2); 请以 -DMAGTILE_BUILD_GL_RENDERER=ON 构建"
    else
        pass "渲染后端已链接 (--gui 退出码 $link_exit, 非 2; 本机无显示环境, 跳过实际渲染)"
    fi
fi
