...
- 生成时间: 2026-08-25 17:49 UTC
- 基线提交: `95c26cd` (`cursor/magtile-studio-foundation-a95b`)
...
工程侧判定: 软件门禁已达上限, 与 `RELEASE_GATE_STATUS.md` (基线 `5b915a0`) 结论一致。全部自动可探测项 (含三个长跑项 R4/R5/R17) 全绿; 唯二失败 R6/R7 为 D4+ 实物复核缺口, 按设计须用户实搭清零 (`docs/PHYSICAL_REBUILD_CHECKLIST.md`), 不属工程可修复范围。

## 2. 逐项结果

| 检查 | 级别 | 结果 | 耗时 | 说明 |
| --- | --- | --- | --- | --- |
| R1 内容体量 | P0 | **PASS** | 0s | 模型 JSON 209 个 >= 门槛 200 (目标区间 200~250) |
| R2 目录/缩略图对账 | P1 | **PASS** | 0s | 模型 209 / 目录登记 209 / 缩略图就绪 209, 三方一致 |
| R3 免费层清单对齐 | P0 | **PASS** | 0s | 免费标签 30 x starter 清单 x core-9 三条断言全过 |
| R4 E2E 冒烟 (全量) | P0 | **PASS** | 35s | 8 项通过 / 1 项 SKIP (详见 §3) |
| R5 发布门禁快检 (全量) | P0 | **PASS** | 35s | 3 个门禁关卡全过 (详见 §4) |
...
| R16 儿童友好文案守卫 | P0 | **PASS** | 1s | 全库文案扫描零红线 |
| R17 D4+ 扰动仿真抽检 (全量) | P1 | **PASS** | 9s | 10/10 全绿, 每模型 --jitter 50 (详见 §5) |
...
| 路径 | 结果 | 耗时 |
| --- | --- | --- |
| E2E-01a CLI 启动冒烟 (catalog 13 片型) | PASS | 0s |
| E2E-11a 免费层清单对齐 (verify_free_tier) | PASS | 1s |
| E2E-11b CLI 免费筛选对账 (--free-only, 30 个) | PASS | 0s |
| E2E-06a CLI 免费模型教程步进 (beach_hut_01, 44 片对账) | PASS | 0s |
| E2E-17a 跨端存档键契约 (CLI 写 -> sqlite 直读 + 全量互通断言) | PASS | 0s |
| E2E-QT Qt 无头冒烟 (test_qt_smoke.sh, offscreen) | PASS | 14s |
| E2E-12a Qt 进度页深链 (--smoke-open-progress) | PASS | 5s |
| E2E-04a/09a/11c/12b Qt 按钮级路径冒烟 | PASS | 15s |
...
| 2. 弱磁严格档全库巡检 (strict) | PASS | 209 模型全绿, strict 零警告审计 + 逐步装配质检 |
...
抽样规则: D5 全数优先 + 大体量 D4 按总片数降序补足, 目标 10 个; 每模型 `validate --jitter 50`。
...
10/10 全绿 (含此前修复过抖动敏感问题的结构族代表), 软件侧不豁免 S1/S2 实搭。

## 6. R6/R7 失败详情 (预期硬闸门, 非工程可修复)

- **R6**: 实物抽样包缺口 10/10 —— D4+ 45 个全部待复核, 抽样命中 S1=0 / S2=1 / S3=9, 预计实搭总耗时约 750 分钟。已标注 `physical_verified` 的 3 个 D3 模型 (castle_foundation_01 / great_wall_01 / tokyo_tower_01) 一致性核对通过。
- **R7**: 扫描 209 模型, D4+ 共 45 个: 已复核 0, 待复核 45 (`--fail-on-pending` 生效)。

两项均为 L3 实物复核硬闸门: 需用户按 `docs/PHYSICAL_REBUILD_CHECKLIST.md` 实搭后回填 `physical_verified` 标记 (缩减流程见 `docs/USER_HANDOFF.md` §4.3)。本次运行无任何非预期 / 工程可修复的失败项。
