...
| 教程播放器 §6 ★ | 步骤导航/高亮/ghost/相机 已可用 | `DONE`（3D 视口 + 步骤导航/高亮/ghost/轨道相机/退出自动存档, 与 GL 版共用场景渲染层与存档；🔊 步骤朗读 + 4-6 岁自动朗读已接 QT-4 骨架；每步星星反馈 + 进度条 10% 里程碑小星已接 QT-4；触屏双指手势、🔊 波形动画 `PLANNED`） | QT-3 |
