...
| 界面 | 章节 | 实现状态 |
|------|------|----------|
| 首页 / 模型库 | §5 | `IN_PROGRESS`（GL 版模型库窗口已可运行：`magtile_app library --dev-gui`（内部开发工具，原 `--gui`），卡片网格/进度徽标已有；Qt 版筛选器已接入：难度/主题/「只用核心 9 片」/「我能搭的」+ 筛选空态，见 QT_UI_PLAN.md QT-1；分龄卡片密度与筛选器收放已在 Qt/GL 双端接入（4–6 超大卡片无筛选 / 7–9 难度+主题 / 10+ 全量筛选+紧凑卡片，读 SQLite 年龄段设置自动切换）；「我能搭的」空态推荐 3 个可搭模型 DONE） |
| 模型详情 | §5.4 | `IN_PROGRESS`（Qt 版详情页 DONE：难度/片数/步数、预计用时「🕒 大约 N 分钟」温和档位估算（4-6 岁更大字，缺片时照常显示）、BOM 清单对照库存缺片琥珀提示、套装分层标签、收藏、「开始搭建」大按钮、可旋转 3D 预览（复用教程视口只读 previewMode，见 QT_UI_PLAN.md QT-1）；GL 版仍从库直接进教程） |
...
**实现状态**：Qt 版详情页 DONE（难度星/片数/步数、预计用时「🕒 大约 N 分钟」（纯函数档位估算：步数 ×1.5 分钟 + 片数 ×0.1 分钟归整到 5/10/15/20/30/45 分钟档、平票取小档不假精确，4-6 岁启蒙模式更大字，与缺片状态无关照常显示——用时是信息不是门槛）、BOM 清单逐片型对照库存（「✓ 够用」/「缺 N 片」琥珀提示 + 替代建议文案）、「核心 9 片就能搭 / 会用到扩展片」套装分层标签、收藏切换、进行中/已完成进度回显、「开始搭建/继续搭建」大按钮（高 64、占宽 80%）、可拖动旋转的 3D 预览（复用教程视口只读 previewMode），见 QT_UI_PLAN.md QT-1）；GL 版无详情页。
...
商用儿童安全模式已在桌面模型库（GL 内部工具 `library --dev-gui`）落地并有测试：
...
- **冒烟测试**：`library --dev-gui --parent-gate` 深链直开门界面，`library_gui_smoke` 无头渲染并截图校验。
