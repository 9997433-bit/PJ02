...
- **套装分层 `tier`**: `core` = 核心片型 (实物基础套装, 免费模型库默认只用这些); `extended` = 扩展片型 (进阶内容可用)。
- **语义标记 `hollow` / `variant` / `wheeled`**: 仅供教程文案、BOM 与渲染区分外观; **物理校验与拼接规则一律使用外框多边形**, 不因镂空/车轮而改变。

## 2. 核心片型 (tier = core, 共 9 种)

| 标识 (type) | 中文名 | 外形尺寸 | 磁力边 | 语义标记 | 用途说明 |
| --- | --- | --- | --- | --- | --- |
| `square` | 正方形 | 边长 1 | 4 边全带 | - | 地面与墙体的主力形状 |
...
| `isosceles_triangle` | 等腰三角形 | 底 1, 高 1 | 3 边全带 | - | 尖塔与装饰 |
| `rectangle` | 长方形 | 2 x 1 | 4 边全带 | - | 长边与两个正方形拼接等长 |
| `wheel_base` | 车轮底座 | 外框同长方形 (2 x 1) | 4 边全带 (长边长 2, 短边长 1) | `wheeled: true`, `variant: "wheeled"` | 底面装有滚动车轮, 作车辆底盘; 顶面/侧面拼接与物理校验按长方形处理 |

核心片型清单 (中文): **正方形、大正方形、窗格方、门框方、等边三角形、直角三角形、等腰三角形、长方形、车轮底座**。

## 3. 扩展片型 (tier = extended, 共 4 种)

| 标识 (type) | 中文名 | 外形尺寸 | 磁力边 | 用途说明 |
| --- | --- | --- | --- | --- |
...
| 标记 | 类型 | 含义 |
| --- | --- | --- |
| `tier` | 字符串 | 套装分层: `core` / `extended`; 免费模型库默认只使用 core 片型 |
