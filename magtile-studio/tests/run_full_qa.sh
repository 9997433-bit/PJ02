...
#    4. 模型库全量质检       (难度感知片数下限: D1 >= 20, 其余 >= 40;
#       D1 下限取 CONTENT_STRATEGY 2.1 节片数带 [20,28] 下沿 + validate 全绿)
...
# 片数下限难度感知: D1 用 CONTENT_STRATEGY 2.1 节片数带下沿 20 (脚本内固定),
# 此处传入的 40 只约束非 D1 模型。
run_stage "模型库全量质检 (难度感知片数下限)" \
