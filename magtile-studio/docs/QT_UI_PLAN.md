...
| **QT-1 模型库完整** | 筛选器（难度/主题/只用核心 9 片/我能搭）、分龄卡片密度（§2）、"继续上次"直达教程、模型详情页（BOM 缺片提示, §5.4） | QT-0 | `IN_PROGRESS`（筛选器 + 筛选空态 + 详情页 + 收藏 + "继续上次"直达详情 + 详情页 3D 可旋转预览（复用教程视口只读 previewMode）+ 分龄卡片密度与筛选器收放（§2 三档）+「我能搭的」空态推荐 3 个可搭模型 DONE；预计用时 PLANNED） |
...
| **QT-3 3D 教程播放器** | `QQuickFramebufferObject` 集成 `magtile_render_gl`：步骤导航/高亮/ghost/轨道相机上屏；退出自动存档 | QT-1 | `DONE`（核心屏 ★：场景绘制抽成 GL/Qt 共用的 `GlSceneRenderer`，视口/导航/高亮/ghost/轨道相机/自动存档 + `--smoke-open-model` 无头冒烟已落地；详情页 3D 预览已以只读 previewMode 复用本视口（QT-1）；触屏手势 PLANNED） |
| **QT-4 反馈与庆祝** | 每步星星反馈、完成庆祝页、成就墙 GUI、QtTextToSpeech 朗读（§4.2/4.3） | QT-3 | `IN_PROGRESS`（完成庆祝页（彩带/星星/成就卡/再搭一次/回模型库, 减少动效降级）+ 完成链路 `completeBuild -> buildCompleted`（写存档完成 + 首次完成成就, 与 GL 版同口径）+ QtTextToSpeech 步骤朗读（🔊 按钮 + 4-6 岁自动朗读 + 设置页「步骤朗读」开关, `tts_enabled` 契约收口 progress/ui_settings, 可选依赖静默降级）+ `--smoke-complete-model` 冒烟 DONE；进度页「我的作品」（三格统计 + 进行中/已完成/收藏列表, 行点击直达详情）+ 成就墙（首页温和统计卡片入口 + 徽章墙全览, 未解锁灰色剪影 + 达成条件, 按完成数 1/3/10/30 分档）+ `--smoke-open-progress` 冒烟 DONE；每步星星反馈动画、进度条 10% 小星、「再搭一个」推荐、🔊 波形动画 PLANNED） |
...
| 模型详情 §5.4 | 无 | `DONE`（3D 可旋转成品预览（复用教程视口只读 previewMode, 拖动旋转/滚轮缩放, 不写存档）/BOM 对照库存缺片琥珀提示/套装分层标签/收藏/开始搭建大按钮）；预计用时 `PLANNED` | QT-1 |
