...
> 只做「工程 vs 你」的分工切面。
>
> **状态快照增补 (2026-08-26, 基线 `9aa146d` → 内容批 P `53615ea`)**:
> **路径 B 难度配额解冻已完成**
...
> `531860e`/`d78f419` + R9 放置重排 `acdc834`)。内容批 P (`53615ea`) 随后
> 置换入库 10 个扩展模型 (总量保持 250), 新增 D4 `expansion_orb_01`,
> D4+ 全集 51 → **52** (46 D4 + 6 D5)。全量 QA 42 子关卡 **0 失败**
> (CTest 557/557), 抗扰动 **52/52 ×
> 50 轮全绿**; `run_release_gate.sh --full --fail-on-pending` **唯一红灯 =
> L3 实物 0/52**。实物排产三件套已刷新至 52 口径 (`ad6d35c`): **必搭 41 个
> ≈ 52.8h + 可缓建 11 个 ≈ 12.8h** (§4.3 工时以此为准)。剩余阻塞只有两条:
...
> 唯二 P0 FAIL。
>
> **治理波次已合入** (`b369bad` 基线): ① **series 归类回填收官
> 250/250** (矩阵内 176 + 矩阵外聚桶 74, 缺失/非法 0; 词表
> `data/content_series_map.json` 权威化), 机检
> `tools/check_content_series.py --strict` 落地为就绪探测 **R18** +
> CTest 硬闸门 `content_series_gate` (随全量回归常开, 每次 push 即
> 拦截漏归类/词值走样) + QA 可选关卡 20; ② **D3 冻结硬闸门**
> `tools/check_difficulty_quota.py` (D1<20 或 D5<6 即冻结生效,
> `--batch` 批次拦截 D3 新模型 + 策展人白名单豁免, `--strict` CI
> 告警闸) 与内容批 PR 一键机检 `tools/review_content_batch.sh`
> (五道阻断关卡, 用法 [CONTENT_STRATEGY.md](CONTENT_STRATEGY.md)
> §4.3) 入库; ③ 矩阵进度**机器快照**
> [reports/CONTENT_MATRIX_PROGRESS.md](reports/CONTENT_MATRIX_PROGRESS.md)
> 自动生成 (`update_model_catalog.py --matrix-report`, 13 主题 ×
> D1~D5 对照终态 520, 现状 176/520 = 34%)。**注意**:
> `run_release_gate.sh --full` 档现为四道发布专项环境变量全开
> (免费层 / strict 巡检 / 系列归类 / 难度配额), 其中**难度配额
> strict 守卫按预期红灯** (D3 冻结生效 —— D1 0/20, D5 1/6): D1≥20
> 且 D5≥6 补齐前 `--full` 保持红属预期告警, **不改变 R6/R7 唯二 P0
> 阻断口径**。
...
代劳的自动侧 P0 缺口, 但 L2 三层流程已把其中的人手量大幅缩减 (见
§4.3); 10 项 SKIP = 快检刻意跳过的 R4/R5/R17 三个长跑项 (全量档会
实跑) + M1~M6 六个纯人工提醒项 (即 §4 的你的清单, 不参与自动判定) +
该次实跑环境无构建目录 R10 记 SKIP。合计项 24 → 25 系治理波次新增
**R18 系列归类机检**所致 (首跑即绿; 此前 23 → 24 为 L2 批次 R17)。
...
| 3 | `tools/physical_family_pack.py` 结构族聚类 (每族一个代表实搭, 削减重复) | §8 S1/S2 | **已交付** `b093b6d` (交付时 209 模型 → 154 结构族; 与风险分探测链会师 `6dff7cb`); **250 基线刷新** `ced770c` (185 族, 多成员 52) |
...
结构族报告交付时 209 → 154 族 ✅ —— **250 基线刷新** (`ced770c`,
...
250 → 185 族 (多成员 52), 族去重后必搭 36 个 / 可缓建 10 个 (省 21%),
Top3 与 234 库持平 (`skyscraper_01` / `suspension_bridge_01` /
`lighthouse_01`); D4+ 默认 `--jitter 50` 全绿 ✅ (`run_strict_audit.sh
--jitter-only --jitter require` 45/45, 基线 `5b915a0`; 扩容至 250 后
D4+ 全集 46 —— 新增批旗舰 `stonehenge_01` D4, 其余 15 个全部 D2/D3
且逐个 `--jitter 50` 全绿入库, 250 基线 46x50 全绿见
...
**新口径 (2026-08-25 L2 决议; 计数已随内容批 P 更新至 52)**: 你**不必把全库
250 个模型全搭一遍, 也不再逐个搭完 52 个 D4+ 全集** (46 D4 + 6 D5,
配额批新增 5 个 D5, 内容批 P 新增 `expansion_orb_01` D4)。实物验证升级为三层流程 (对账口径
...
(内容批 P 后 52 口径 `ad6d35c`: **待复核 52 = 必搭 41 ≈ 52.8h + 可缓建 11
≈ 12.8h**; 三件套已在批 P 基线 `53615ea` 重导出, 此前 D5 返工加固的
片数扩规模已计入风险分, 无陈旧告警)。三工具与 `--jitter` 均为 L2 批次交付物
...
| R7 缩减集清零: risk Top 15 + 结构族代表 (扣除与抽样包重合部分) | 同上; **非代表模型不用搭** —— 由「同族代表实搭通过 + 第二层 jitter 全绿」覆盖; 可多人按模型切分 (每模型一人走完全程) | 同上 | 同上 §1/§6 | **以两份报告实跑清单为准 (已刷新至内容批 P 后 52 口径 `ad6d35c`): 族去重后必搭 41 个 ≈ 3170 分钟 (约 52.8 小时), 可缓建 11 个 ≈ 770 分钟 (约 12.8 小时, 省 20%)** (全集口径 52 个 ≈ 3940 分钟 ≈ 65.7 小时; 缓建采纳与否是策展/QA 政策决定, 见 [reports/PHYSICAL_FAMILY_PACK.md](reports/PHYSICAL_FAMILY_PACK.md)) |
...
**工时汇总** (单人动手, 可多人并行压缩): 实物缩减集必搭 41 个
≈ 52.8 小时 (内容批 P 后 52 口径排产队列实跑 `ad6d35c`; 可缓建 11 个另计约
12.8 小时; 全集口径 52 个 ≈ 65.7 小时) + 实机/真机/签核约 3~4 个工作日 +
行政动手约 2~3 个工作日 + 付费验收约 1 个工作日 + 法务协调约 1 个
工作日 ≈ **13~15 个工作日量级** (全集 52 个全搭则 ≈ 15~17);
