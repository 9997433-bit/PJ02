...
> 数据资产打包与首启解包、商用视觉抛光 (对齐桌面 Qt Theme 口径:
> colors/dimens 设计令牌统一主色/字号层级/间距/触控 ≥48dp; 模型库
> 加载失败与空目录温和空态 +「再试一次」, 技术细节只进 logcat;
> 筛选栏分组留白; 分龄卡片密度三档 —— 10+ 进阶换紧凑卡; 减少动效
> 双通道 —— 应用内共享 `reduce_motion` 设置键 (桌面设置页开关同键,
> 经 ui_settings JNI) 与系统动画设置任一命中即降级: 列表瞬时定位 /
> 点按反馈退静态按压色 / 3D 视口呼吸描边定格最亮帧 + 脏帧按需重绘)。
...
- 3D 教程视口 (骨架已落地, 剩余缺口):
  ...
  - **缺口**: 视口 MSAA 抗锯齿 (桌面 4x, Android 当前用默认
    ...
    常规档按需渲染节电 (减少动效档已切脏帧模式按需重绘; 常规档
    仍 RENDERMODE_CONTINUOUSLY 连续重绘驱动呼吸动画, 后续可改
    脏帧模式 + 呼吸期定频重绘); 「转一转视角」引导态 (第 0 步
    空场景) 与视角重置按钮。减少动效已双通道接通 (不再是缺口):
    应用内共享 `reduce_motion` 设置键 (桌面 GL/Qt 设置页开关同键,
    经 ui_settings JNI 读取, `MagTileNative.reduceMotion()`) 与
    **系统动画设置** (MotionPrefs, 动画时长/过渡缩放为 0) 任一
    命中即降级 —— 列表瞬时定位 / 点按静态按压色 / 呼吸描边定格
    最亮帧。
