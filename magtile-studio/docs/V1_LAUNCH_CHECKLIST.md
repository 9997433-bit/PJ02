...
(2026-08-25 17:38 UTC, `5b915a0` 基线, 209 模型时点); 全量就绪探测留痕:
[reports/READINESS_FULL_2026-08-25.md](reports/READINESS_FULL_2026-08-25.md)
(17:49 UTC, `95c26cd` 基线)。内容扩容至 **234** (`3d24d74`) 后, 上述两报告
与 strict 巡检深报告随并行刷新批复跑, 以各文件最新快照为准; L2 风险
报告与结构族包**已刷新至 234 基线** (`50a3f4c`, 见 §8)。
...
| C2 | 全库质量门禁全绿 (16 关 QA + strict 零未豁免警告) | P0 | Auto (R5) | `tests/run_full_qa.sh` / `tools/run_strict_audit.sh`; 巡检深报告 [reports/STRICT_AUDIT_2026-08-25.md](reports/STRICT_AUDIT_2026-08-25.md) (209 模型时点终态, 234 基线刷新批复跑中); 最新全库实跑留痕 [reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md) (`5b915a0` 基线, 209 模型; 234 基线刷新批复跑中) | ✅ 全库 234 模型 strict 双档零未豁免警告 (最近内容并入批 `3d24d74` 全库 QA 36 关卡全绿: test_all_models 234/234 + strict 零未豁免 + 唯一性 27261 对 0 警告; D4+ 全集 45 个扩容后不变—新增批全部 D2/D3, jitter 45/45 全绿结论沿用 `5b915a0` 基线; 唯一豁免 `suspension_bridge_01` 已文档化) |
...
| # | 待办 | 优先级 | 探测 | 载体 / 依据 | 状态 (2026-08-25) |
| --- | --- | --- | --- | --- | --- |
| S0 | 第二层虚拟物理验证全绿 (D4+ 默认 `--jitter 50` + 风险报告 + 自动标记) | P0 | Auto (R17) | `magtile_app validate --jitter N` (蒙特卡洛 ±1.5mm/±2° 逐步扰动, 失败记 F08 类) + `tools/physical_risk_report.py` + [BUILD_VERIFICATION.md](BUILD_VERIFICATION.md) §2 触发条件代码化; 门禁接入 `tools/run_strict_audit.sh` / `tools/run_release_gate.sh`; jitter 夹具与 risk report 单测随 ctest; 就绪探测载体 `tools/check_v1_readiness.sh` R17 (P1 报告档): D4+ 确定性抽检 —— D5 全数优先 + 大体量 D4 按总片数降序补足 (默认 10 个, `--jitter-sample` 可调; 与 `physical_sample_pack.py` 同一风险代理口径), 逐个 `validate --jitter 50`; 慢项 `--quick` 记 SKIP 而**全量签核必跑**: 二进制缺失 / `--jitter` 特性不可用 / 抽样为空一律显式 FAIL 不静默跳过; `physical_risk_report.py` 落地后抽样源可切换其高风险 Top 子集 | ✅ 工具链 + 4 边缘模型加固全部入库; D4+ jitter 45/45 全绿 (`5b915a0` 基线; 扩容至 234 后 D4+ 全集仍为 45, 新增批全部 D2/D3 且逐个 `--jitter 50` 全绿入库); `run_release_gate --full --l2` QA+L2 全绿 (实跑留痕 [reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md), 234 基线刷新批复跑中) |
