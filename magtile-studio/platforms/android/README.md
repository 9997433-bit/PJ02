...
> 联动系统动画设置: 列表瞬时定位 / 点按反馈退静态按压色 / 3D 视口
> 呼吸描边定格最亮帧 + 脏帧按需重绘)。
...
- 3D 教程视口 (骨架已落地, 剩余缺口):
  ...
  - **缺口**: 视口 MSAA 抗锯齿 (桌面 4x, Android 当前用默认
    ...
    空场景) 与视角重置按钮; 减少动效目前联动**系统动画设置**
    (MotionPrefs, 动画时长/过渡缩放为 0 即降级 —— 列表瞬时定位 /
    点按静态按压色 / 呼吸描边定格最亮帧), 应用内共享
    `reduce_motion` 设置键 (桌面设置页开关同键) 待 ui_settings
    JNI 链路接通。
