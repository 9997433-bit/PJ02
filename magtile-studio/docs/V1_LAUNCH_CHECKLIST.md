...
| --- | --- | --- | --- | --- | --- |
| C1 | 模型库体量达 200~250 | P0 | Auto (R1) | `data/models/*.json` 计数; [CONTENT_STRATEGY.md](CONTENT_STRATEGY.md) 产能与主题池 | ✅ 250 个 JSON (批 P 置换 +10 扩展装 (1 D1 + 7 D2 + 1 D3 白名单 + 1 D4) / −10 矩阵外 D3, 总量保持 250, `53615ea` 按 [reports/BATCH_P_MERGE_PLAN_2026-08-26.md](reports/BATCH_P_MERGE_PLAN_2026-08-26.md) 摘取; 此前配额批 +25/−25、内容批 F~I 234→250、批 A~E 209→234; R1 门槛 200 恒过, **200~250 目标区间上限达成**) |
| C2 | 全库质量门禁全绿 (16 关 QA + strict 零未豁免警告) | P0 | Auto (R5) | `tests/run_full_qa.sh` / `tools/run_strict_audit.sh`; 内容批 PR 一键机检 `tools/review_content_batch.sh` (五道阻断关卡: strict 校验 / D3 冻结 / series 归类 / core-9 分层 / 唯一性抽查, 用法 [CONTENT_STRATEGY.md](CONTENT_STRATEGY.md) §4.3); 巡检深报告 [reports/STRICT_AUDIT_2026-08-25.md](reports/STRICT_AUDIT_2026-08-25.md) (**已刷新至 250 基线** `2b2c4ff`: 249 通过 + 1 白名单豁免, 逐步装配 250/250, D4+ 46x50 全绿); 最新全库实跑留痕 [reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md) (**已刷新至批 P 后基线** `53615ea`: 全量 QA 0 失败, 唯一红灯 L3 52 待复核) | ✅ 全库 250 模型 strict 双档零未豁免警告 (批 P 后基线 `53615ea` 全量 QA 42 子关卡 0 失败: CTest 557/557, strict 巡检 + 免费层对齐 + 系列归类 + 难度配额 strict 四道发布专项全开全绿, 唯一性 0 警告, 儿童文案守卫 301 文件 8493 段全绿; D4+ 全集 51 → 52 —— 批 P 净增 1 个 D4 `expansion_orb_01`, 抗扰动 **52/52 x 50 轮全绿**; 批 P 10 模型经 `review_content_batch.sh` 五道闸在本线基线过检后摘取入库, 4 个 D2 返工扩规模满足难度感知下限, `sector_rotunda_01` 走 D3 白名单策展豁免 (`reports/BATCH_P_D3_WHITELIST.txt`); 此前配额批 5 个红项清零路径留痕不变 —— 治理决策 `b925eba` / D5 扩规模 `531860e`/`d78f419` / R9 重排 `acdc834`; 唯一 strict 豁免 `suspension_bridge_01` 已文档化) |
| C3 | 模型库目录全量登记 + 缩略图全覆盖 | P0 | Auto (R2) | `tools/update_model_catalog.py` / `tools/generate_thumbnails.py` | ✅ JSON 250 / 目录登记 250 / 缩略图就绪 250 三方对账一致 (批 P 后 `53615ea` 重登记; 难度分布 D1 x21 / D2 x30 / D3 x147 / D4 x46 / D5 x6 —— **D1~D5 全难度覆盖保持**) |
...
`6dff7cb`) / 结构族 `b093b6d` (交付时 209 模型 → 154 族; **批 P 后
52 口径刷新** `8974b4b`: 195 族, 多成员 43, 必搭 41 / 可缓建 11 省 20%,
...
D4+ `--jitter 50` 全绿 (234 基线 45/45; 250 基线全集 46; 配额批后
全集 51 —— 新增 5 个 D5 (`stellar_launch_gantry_01` /
`strait_rainbow_bridge_01` / `royal_citadel_01` / `giant_ferris_wheel_01` /
`marble_grand_cascade_01`); **批 P 后全集 52** —— 净增 1 个 D4
`expansion_orb_01`, 52/52 x 50 全绿见
[reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md)),
`run_release_gate --full --fail-on-pending` 软件侧全绿 (唯一预期红 L3 0/52, 见
[reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md), **已刷新至批 P 后基线** `53615ea`)。

| # | 待办 | 优先级 | 探测 | 载体 / 依据 | 状态 (2026-08-26) |
| --- | --- | --- | --- | --- | --- |
| S0 | 第二层虚拟物理验证全绿 (D4+ 默认 `--jitter 50` + 风险报告 + 自动标记) | P0 | Auto (R17) | `magtile_app validate --jitter N` (蒙特卡洛 ±1.5mm/±2° 逐步扰动, 失败记 F08 类) + `tools/physical_risk_report.py` + [BUILD_VERIFICATION.md](BUILD_VERIFICATION.md) §2 触发条件代码化; 门禁接入 `tools/run_strict_audit.sh` / `tools/run_release_gate.sh`; jitter 夹具与 risk report 单测随 ctest; 就绪探测载体 `tools/check_v1_readiness.sh` R17 (P1 报告档): D4+ 确定性抽检 —— D5 全数优先 + 大体量 D4 按总片数降序补足 (默认 10 个, `--jitter-sample` 可调; 与 `physical_sample_pack.py` 同一风险代理口径), 逐个 `validate --jitter 50`; 慢项 `--quick` 记 SKIP 而**全量签核必跑**: 二进制缺失 / `--jitter` 特性不可用 / 抽样为空一律显式 FAIL 不静默跳过; `physical_risk_report.py` 落地后抽样源可切换其高风险 Top 子集 | ✅ 工具链 + 边缘模型加固全部入库; D4+ jitter 全绿 (234 基线 45/45 复验 `3d24d74`; 250 基线全集 46; 配额批后全集 51 —— 新增 5 个 D5 逐个 `--jitter 50` 全绿入库 (含 2 个扩规模返工件 `531860e`/`d78f419` 与 R9 重排件 `acdc834` 复验); **批 P 后 D4+ 全集 52** —— 净增 1 个 D4 `expansion_orb_01`, 52/52 x 50 全绿见 [reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md)); `run_release_gate --full` QA 全绿 (实跑留痕 [reports/RELEASE_GATE_STATUS.md](reports/RELEASE_GATE_STATUS.md), **已刷新至批 P 后基线** `53615ea`) |
| S1 | 缩减人手集实搭签核: **risk Top 15 + 结构族代表** (并集去重, 清单以两份报告实跑输出钉死) | P0 | Auto 报告 (R6 + R17) + Manual 实搭 | `tools/physical_risk_report.py` Top 15 人手建议 (实跑报告 [reports/PHYSICAL_RISK_REPORT.md](reports/PHYSICAL_RISK_REPORT.md), **已刷新至批 P 后 52 口径** `8974b4b`: 250 全扫, L2 标记 145 / l2_required 155, Top3 `skyscraper_01` 63.9 / `stellar_launch_gantry_01` 61.8 / `marble_grand_cascade_01` 57.6, Top15 清单 ≈ 22.0 小时) + `tools/physical_family_pack.py` 结构族代表 (实跑报告 [reports/PHYSICAL_FAMILY_PACK.md](reports/PHYSICAL_FAMILY_PACK.md), **已刷新至 52 口径**: 195 族, 多成员 43, 必搭 41 ≈ 52.8h / 可缓建 11 ≈ 12.8h 省 20%, 全集 ≈ 65.7h; 排产单 [reports/PHYSICAL_REVIEW_QUEUE.md](reports/PHYSICAL_REVIEW_QUEUE.md) 52 行同口径); 规程 [PHYSICAL_REBUILD_CHECKLIST.md](PHYSICAL_REBUILD_CHECKLIST.md) + 上手指南 [reports/PHYSICAL_REVIEW_USER_GUIDE.md](reports/PHYSICAL_REVIEW_USER_GUIDE.md) + 工作单 [reports/PHYSICAL_SIGNOFF_WORKSHEET.md](reports/PHYSICAL_SIGNOFF_WORKSHEET.md); 原 V1 抽样包 10 个 ([reports/PHYSICAL_SAMPLE_V1.md](reports/PHYSICAL_SAMPLE_V1.md), `tools/physical_sample_pack.py --fail-on-missing-sample`) 预计与 Top 15 高度重合, 作首批排产兼第二层校准数据 | ⬜ 0/52 已复核 —— 人手范围由报告钉死 (必搭 41 个), **不必 250 全搭**; 路径 A 第 1 场开工单已备 [reports/PATH_A_SESSION_01_SKYSCRAPER.md](reports/PATH_A_SESSION_01_SKYSCRAPER.md) |
