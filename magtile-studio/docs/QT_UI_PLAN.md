...
| **QT-4 反馈与庆祝** | 每步星星反馈、完成庆祝页、成就墙 GUI、QtTextToSpeech 朗读（§4.2/4.3） | QT-3 | `IN_PROGRESS`（完成庆祝页（彩带/星星/成就卡/再搭一次/回模型库, 减少动效降级）+ 完成链路 `completeBuild -> buildCompleted`（写存档完成 + 首次完成成就, 与 GL 版同口径）+ QtTextToSpeech 步骤朗读骨架（🔊 按钮 + 4-6 岁自动朗读 + `tts_enabled` 开关挂钩, 可选依赖静默降级）+ `--smoke-complete-model` 冒烟 DONE；进度页「我的作品」（三格统计 + 进行中/已完成/收藏列表, 行点击直达详情）+ 成就墙（首页温和统计卡片入口 + 徽章墙全览, 未解锁灰色剪影 + 达成条件, 按完成数 1/3/10/30 分档）+ `--smoke-open-progress` 冒烟 DONE；每步星星反馈动画、进度条 10% 小星、「再搭一个」推荐、🔊 波形动画 PLANNED） |
