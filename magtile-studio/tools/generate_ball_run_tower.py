...
结构总览 (世界单位: 1.0 = 正方形磁力片边长):
  ...
  - 东线轨道 (顺时针绕塔): 30 度坡道 -> 转角平台 -> 30 度坡道 ->
    转角平台 -> 冲线坡道落地, 每段坡尾由独立栈桥墩接住               12 片
  - 西线轨道: 与东线严格镜像 (T11), 双轨竞速                        12 片
...
flat("cor_e1", X1, 0, 2.0, "yellow")      # 转角台: 西缘与坡尾/墩顶三件互吸
...
step(
    "东线第 1 段 (整段成组安装): 先在塔东侧立双层灰色栈桥墩, 再把橙色坡道顶边"
    "整边吸在发球台东缘出珠口, 坡尾搭上墩顶, 最后铺黄色转角台压住墩顶与坡尾。",
    ["tre_e1a", "tre_e1b", "ramp_e1", "cor_e1"],
    highlight=["plat_1_0", "tw2_e_0"],
    tip="坡道-桥墩-转角台三件互相吸住才算装好: 这是一个三角刚性节点。",
)
step(
    "东线第 2 段: 转角台外缘与北缘装红色挡珠三角 (逼弹珠向南转弯), 立第二座"
    "栈桥墩, 南向坡道从转角台南缘下探, 再铺第二块转角台。",
    ["rail_e1_out", "rail_e1_n", "tre_e2", "ramp_e2", "cor_e2"],
...
step(
    "西线第 1 段: 与东线镜像 —— 塔西侧立双层栈桥墩, 坡道吸住发球台西缘出珠口, "
    "铺转角台。",
    ["tre_w1a", "tre_w1b", "ramp_w1", "cor_w1"],
    highlight=["plat_0_1", "tw2_w_1"],
    tip="对照东线检查: 西线每一件都应出现在镜像位置上。",
)
step(
    "西线第 2 段: 转角台装挡珠三角 (逼珠向北转), 立栈桥墩, 北向坡道下探, "
    "铺第二块转角台。",
    ["rail_w1_out", "rail_w1_s", "tre_w2", "ramp_w2", "cor_w2"],
    highlight=["cor_w1", "tre_w1b"],
    tip="西线绕塔方向与东线相反 —— 两条轨道像双螺旋一样对称盘下。",
)
step(
    "西线冲线段与装饰: 装最后的挡珠三角与冲线坡道 (落向西接珠港); 两港各挂"
    "六边形靶标, 港口立粉色冲线旗门 —— 双轨竞速滚珠塔完工, 放珠开赛!",
    ["rail_w2_out", "rail_w2_n", "ramp_w3", "hex_e", "ban_e", "hex_w", "ban_w"],
