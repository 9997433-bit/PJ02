...
QT-4 补充说明（进度页 + 成就墙）：首页「我的进度」大按钮与**温和统计卡片**（有作品后儿童侧可见，"已完成 N 个模型 · 点亮 N 枚徽章"，只报喜不催促 §4.3）同路由直达 `ProgressPage`「我的作品」（§5.3 儿童可达，无家长门）：三格统计（已完成/进行中/收藏）+ 成就墙条带 + 进行中（进度条 + 第 x/y 步 + 用时）/ 已完成（完成日期 + 用时 + 片数）/ 我的收藏三个列表，任意行点击直达模型详情（继续搭建/再搭一次都走详情页既有大按钮 → `startBuild` 统一路由）；空态温和引导「去模型库挑一个」（`stack.pop(null)` 后落 首页→模型库，≤ 2 步回首页 §3）。`AchievementsPage` 成就墙全览：徽章卡片网格，已点亮 = 完成绿卡 + ✓ + 解锁日期，未点亮 = **灰色剪影 + 一句话达成条件**（§7.1 防焦虑：不显示进度百分比）。数据经 `StudioBackend` 新增的 `achievementsList / inProgressList / completedList / favoritesList` 四个只读快照与 `favoriteCount / achievementCount` 属性读进度存档（与 CLI `progress list` 同一份 SQLite，`catalogChanged` 时页面自动刷新；收藏切换即时增量维护）。徽章档位只与搭建行为挂钩（§4.5，按完成模型数 1/3/10/30 分档）：`first_model_completed` 与完成链路写档的成就 id 同名，其余档位按完成数在展示层判定达成（不新增写库触发点，成就触发统一收口留待成就系统完整落地）；存档中额外解锁的成就 id 以通用徽章补列，永不缺席。无头冒烟：`--smoke-open-progress` 启动直开进度页（评审/截图深链），`--smoke-parent-flow` 自动驾驶已扩为 进度页→成就墙→门→家长中心→设置→订阅。
