...
```
         ...
         ╱─────╲
        ╱  L2   ╲  物理仿真抽检 (Jolt/Rapier 站立测试) —— 被标记的模型
       ╱─────────╲
...
- **内容**: `magtile_app validate` 执行的全部检查 —— R1 接地支撑、R2 磁力吸合、R3 无重叠、R4 重心稳定, 覆盖成品与每个教程中间状态; 外加教程一致性质检 (步骤连续、每片恰放一次、引用存在、说明非空)。
...
- **内容**: 将成品及关键中间状态导入刚体物理引擎 (候选: [Jolt Physics](https://github.com/jrouwe/JoltPhysics) 或 [Rapier](https://rapier.rs/)), 磁吸边建模为**可断裂约束** (按品牌标定的最大拉力/力矩), 执行三段测试:
  1. **静置沉降**: 重力下模拟 5 秒, 任何片位移 > 5mm 或约束断裂即失败;
  2. **扰动脉冲**: 对最高点施加水平小冲量 (模拟轻碰), 结构须回稳;
  3. **容差抖动 (蒙特卡洛)**: 对每片位姿注入 ±1.5mm / ±2° 随机误差, 重复 N=20 次沉降, 通过率 ≥ 90% 才算通过 —— 专门捕捉"微小错位累积坍塌"。
- **执行者**: CI 中的独立 Job (仿真管线落地后), 当前阶段由工程师手动运行。
- **成本**: 分钟级/模型。**覆盖**: 被标记的模型 (见第 2 节触发条件)。
- **定位**: L2 是 L1 的**抽检补强**, 不是替代 —— L1 的规则永远先跑、永远全量。
...
**L2 标记触发条件** (满足任一即被标记, 编辑器与 CI 自动判定):

1. L1 产生任何 Warning;
2. 结构高度 > 6 个单位, 或存在连续 ≥ 3 片的垂直墙链 (墙上立墙再立墙);
3. 重心投影到接地凸包边界距离 < `stability_margin` 的 50% (临界稳定);
4. 使用扇形/六边形等低磁力边占比形状承重;
5. 内容设计师在编辑器中手动标记 (见第 5 节)。
...
**适用**: T4/T5 必做; T1~T3 新系列首发模型抽做。**前提**: 监护人书面同意, 全程监护人或测试员在场, 3 岁以下磁铁玩具禁令严格执行, 儿童可随时中止。

| 年龄组 | 形式 | 目标分级 | 通过标准 |
| --- | --- | --- | --- |
| 4~6 岁 | 家长共读教程, 儿童动手, 家长可扶稳底座但不代放 | T1~T2 | 完成率 ≥ 80%, 求助 ≤ 3 次/10 步, 无挫败弃搭 |
| 7~9 岁 | 独立跟随教程, 观察员只记录不干预 | T2~T4 | 完成率 ≥ 80%, 卡壳步骤 (>3 分钟) ≤ 2 处 |
| 10+ 岁 | 完全独立, 含拆解重搭 | T3~T5 | 完整通过 3.1~3.4 全部项目 |

观察员记录: 每步是否理解说明 (是/否/求助)、抖动导致的掉片次数、挫败情绪事件、儿童原话反馈。**同一模型至少 2 名目标年龄段儿童**, 结论取较差者。儿童测试暴露的问题优先修改教程 (拆步、加提示、调整放置顺序), 而不是降低通过标准。
...
每种真实失效模式给出: 编码、现场照片描述 (照片按 `assets/failures/<编码>_<模型id>_<序号>.jpg` 归档)、**应当由哪条软件规则拦截** (含尚未实现的规划规则, 引用 PHYSICS_RULES.md 第 5 节)。凡"软件本应拦截却漏过"的实物失效, 一律回填为该规则的回归测试用例。
...
| F03 | 悬挑倾覆 | 外挑阳台放到第 3 片时整体绕支撑边翻倒, 照片为倒塌瞬间连拍 | R4 重心稳定; 大悬挑需规划中的**逐片力矩分析** (§5.1) | L1 / L2 |
...
| F06 | 放置干涉 (够不着) | 四面墙合拢后教程要求在内侧放地台补片, 成人手指伸入即碰倒后墙 | 规划中的**搭建顺序人因校验** (§5.4: 每步可达性、单手可放置) | L3 (未来 L1) |
| F07 | 手抖连锁塌 | 6 岁儿童往双层墙顶放三角城齿, 手腕抖动带倒整面上层墙 | 无法直接拦截; 缓解规则: 高处放置步骤限制 (规划 §5.4) + 每步 ≤ 8 片的步骤粒度规范 | L3 儿童测试 |
| F08 | 错位累积失稳 | 16 步模型搭到第 12 步, 累积 ~5mm 歪斜, 静置 20 秒后自行坍塌 | L2 蒙特卡洛容差抖动测试; 长链结构预警 (第 2 节标记条件 2) | L2 |
...
- 依据第 2 节标记触发条件 (Warning、高墙链、临界重心、弱磁形状承重) 自动点亮 **"需要实物复核"** 标记;
...
4. **L2 仿真 Job**: 对被标记模型运行站立测试 (仿真管线落地前, 此步降级为"检查旁车文件中已有人工 L2 记录")。
