...
物理规则要点 (validate 常规 + strict 双档零警告):
  ...
  - 全部分流构件贴地 (接地即支撑), 最高点发球拱 z=2.0 < 2.5,
    不触发 R8 高层结构条款;
  - 穿廊地坪与分流广场全部平铺接地, 每片至少一条整边吸住桥墩
    底边或邻板 (R2/R7a 天然满足), 接地凸包随之外扩。
...
#    穿廊地坪: 廊下方板 x3 + 西口起点门坪 x1 (整边吸桥墩底边)
# =================================================================
for i in range(4):
...
for i in range(3):
    b.flat(f"deck_{i}", i, 0, 0.0, PLAZA)
b.flat("apron", -1, 0, 0.0, PLAZA)
...
# 分流广场: T 口西侧内角方板 x2 + 东侧外角方板 x2 + 东沿小旗 x2
b.flat("plaza_nw", XP, 1, 0.0, PLAZA)
b.flat("plaza_sw", XP, -1, 0.0, PLAZA)
b.flat("plaza_ne", XE, 1, 0.0, PLAZA)
b.flat("plaza_se", XE, -1, 0.0, PLAZA)
b.crest_ew("flag_n", XE + 1.0, 1, 0.0, RAIL)
b.crest_ew("flag_s", XE + 1.0, -1, 0.0, RAIL)
...
b.step(
    "铺穿廊地坪: 三片灰色方板铺进拱廊门洞下, 西口再接一片起点门坪。",
    ["deck_0", "deck_1", "deck_2", "apron"],
    highlight=["pier_0", "pier_3"],
    tip="地坪整边吸住桥墩底边 —— 从起点门坪望进去, 门洞视线一通到底。",
)
b.step(
...
b.step(
    "铺分流广场: 四片灰色方板补齐 T 口四角, 东沿再立两面红色小旗。",
    ["plaza_nw", "plaza_sw", "plaza_ne", "plaza_se", "flag_n", "flag_s"],
    highlight=["cross_n", "cross_s"],
    tip="广场把 T 口围成一块整地 —— 出界的弹珠也会被兜回分流口。",
)
b.step(
...
b.finalize(
    ...
    description=(
        ...
        "的接珠湾。廊下三片灰色地坪连起点门坪一通到底, T 口四角石板"
        "广场配红色小旗围出分流场地。门框方一片三用 (桥墩/发球拱/"
        "分流门), 分流发生在地面 —— 与高塔类滚珠模型的立体动线完全"
        "不同, 全部 CORE-9。"
    ),
    difficulty=2,
