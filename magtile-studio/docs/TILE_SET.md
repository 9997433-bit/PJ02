...
本文档是 MagTile Studio 全部磁力片片型的权威清单与约定说明。几何数据的唯一事实来源是 `data/tile_catalog.json`, C++ 侧的稳定标识定义在 `include/magtile/core/types.hpp` (`TileType`), 两者必须一一对应 (见第 6 节新增片型清单)。

## 1. 通用约定

- **单位**: 1.0 = 标准正方形边长, 实物约 70mm (`unit_mm`)。
- **顶点**: 位于本地 XY 平面, 逆时针排列; 除扇形以圆心角顶点为本地原点外, 其余形状均以质心为本地原点。
- **磁力边**: 边 i 连接顶点 i 与 i+1, `magnet_edges` 列出带磁条的边索引; 磁力吸合的边长匹配规则见第 4 节 (R2, 见 `docs/PHYSICS_RULES.md`)。
- **套装分层 `tier`**: `core` = 核心片型 (共 9 种, 实物基础套装确认: 5 种基础片 + 大正方形 / 窗格方 / 门框方 / 车轮底座; 免费模型库默认只用这些); `expansion` = 扩展包片型 (进阶内容可用)。产品分层细节与磁石排布见 `docs/TILE_CATALOG.md`。
- **语义标记 `hollow` / `variant` / `wheeled`**: 仅供教程文案、BOM 与渲染区分外观; **物理校验与拼接规则一律使用外框多边形**, 不因镂空/车轮而改变。

## 2. 核心片型 (tier = core, 共 9 种, 依据实物确认)

5 种基础片, 加上大正方形与三种功能变体片 (窗格方 / 门框方 / 车轮底座):

| 标识 (type) | 中文名 | 外形尺寸 | 磁力边 (实物磁石数) | 语义标记 | 用途说明 |
| --- | --- | --- | --- | --- | --- |
...
| `large_square` | 大正方形 | 边长 2 (面积 = 4 小方) | 4 边全带 (每边约 4 颗) | - | 大面积地台与墙面, 每边可吸 1 条长方形长边或 2 片共线小方 (见第 4 节), 显著减少片数 |
...
核心片型清单 (中文): **正方形、等边三角形、直角三角形、等腰三角形、长方形、大正方形、窗格方、门框方、车轮底座** —— 免费模型库默认只使用这 9 种 (见 `docs/CONTENT_STRATEGY.md` 的 core-9 质检规则与 `tools/check_core5_usage.py`)。

## 3. 扩展片型 (tier = expansion, 共 4 种)
...
磁力吸合判定 (R2, `docs/PHYSICS_RULES.md`): **较短磁力边的两个端点都落在较长磁力边线段上** (容差 `connect_tolerance` = 0.02) 即吸合; 等长边时退化为端点两两重合 (整边完全对齐)。

| 边长搭配 | 判定 | 典型场景 |
| --- | --- | --- |
| 1 ↔ 1 (等长) | 整边对齐吸合 (1 对 1) | 小方与小方、小方与三角形边、六边形边 |
| 2 ↔ 2 (等长) | 整边对齐吸合 (1 对 1) | 大正方形边 ↔ 长方形长边 / 另一片大正方形边 / 梯形下底 |
| 2 ↔ 1 (长短搭配) | 短边完整落在长边上即吸合; **一条长 2 的边可同时吸两片共线的长 1 边** (各记一条独立连接, 即 "1 条大边 = 2 条小边") | 大正方形一边吸 2 片小方; 长方形长边/梯形下底吸小方 |
| 错位半搭 | **不吸合**: 短边任一端点悬出长边即判未连接 (磁条对不齐, 实物不稳定) | 两片小方错开半格; 小方半边悬出长方形长边 |

长短边吸合时, R5/R6 静力分析按**实际磁条接触段长度** (即短边长度) 计算承重与力矩预算; 两条共线的 "大边吸小边" 连接归入同一条铰链线, 合计接触长度 2, 与整边 2 ↔ 2 吸合等效。
