...
step(
    "裙楼一层北墙与西墙 (各 4 片), 一层墙体合围。",
    [f"L1_n_{i}" for i in range(4)] + [f"L1_w_{j}" for j in range(4)],
    highlight=["L1_e_3", "L1_s_0"],
...
step(
    "二层其余三面墙 (10 片): 东面与西面各留一扇通风窗, 北面立满。",
...
step(
    "铺设二级露台楼板 (3x3 共 9 片): 从贴西墙一列开始向东铺, 二级塔身封顶。",
    [f"d2_{i}_{j}" for i in range(3) for j in range(1, 4)],
...
step(
    "三级塔身向东北旋位起墙 (7 片): 塔身再退到 2x2, 西面留一扇观景窗。",
