...
| B2 | 真实商店计费接入 (Windows 商店 / Google Play `StoreBillingClient`) | P0 | Auto (R11 探测空实现) | `src/billing/store_billing_client.cpp` (当前开编译宏即 `static_assert` 拒绝) | ⬜ 空实现档, 全部返回 Unavailable, 未接任何真实 SDK |
| B3 | 商店商品配置 + 沙箱付费验收 (真实账号购买 / 恢复 / 退款链路) | P0 | Manual | 商品 id 约定见 `store_billing_client.hpp` 注释; 价格表 [COMMERCIAL_PLAN.md](COMMERCIAL_PLAN.md) §3.1 | ⬜ 依赖 B2 与开发者账号 (L3) |
