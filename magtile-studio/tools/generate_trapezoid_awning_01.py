...
结构总览 (世界单位: 1.0 = 正方形磁力片边长, 墙在北、花园朝南):
  ...
  - 步道包边 (z=0): 平铺梯形 x2 + 扇贝补花砖三角 x3 + 檐前
    三角花 x2                                                    7 片
  - 步道两端收边: 外沿绿篱三角 x2 + 南角花三角 x2                4 片
  合计 40 片, 9 个教程步骤, 4 种磁力片形状 (含扩展梯形 x8)。
...
物理规则要点 (validate 常规 + strict 双档零警告):
  ...
  - 女儿墙尖端 2.87 触发 R8: 墙体横砌环 + 展台双缝连接提供环状
    加固, 无单点失效损失 >= 3 片;
  - 扇贝补花砖平铺接地, 与梯形包边斜边整边互吸 (中央一片同时
    吸两条斜边); 步道延伸方板一边吸长石板短边、一边吸碎石南沿,
    绿篱与角花单片立在延伸板边沿, 剪断只失联 1 片。
...
# =================================================================
# 1. 步道 (y [0,1]): 两片灰色长方形 + 两端延伸方板
# =================================================================
b.flat_rect("walk_w", 0, 0, 0.0, WALK)
b.flat_rect("walk_e", 2, 0, 0.0, WALK)
b.flat("walk_ext_w", -1, 0, 0.0, WALK)
b.flat("walk_ext_e", 4, 0, 0.0, WALK)
...
# =================================================================
# 6. 步道包边 (z=0): 平铺梯形 x2 + 扇贝补花砖三角 x3 + 檐前三角花 x2
#    补花砖把梯形斜边之间的 V 形豁口填平, 扇贝包边连成整条饰带
# =================================================================
...
b.place_tri("scallop_w", "equilateral_triangle",
            (-0.5, -TRAP_H, 0.0), (0.5, -TRAP_H, 0.0), (0.0, 0.0, 0.0), APRON)
b.place_tri("scallop_c", "equilateral_triangle",
            (1.5, -TRAP_H, 0.0), (2.5, -TRAP_H, 0.0), (2.0, 0.0, 0.0), APRON)
b.place_tri("scallop_e", "equilateral_triangle",
            (3.5, -TRAP_H, 0.0), (4.5, -TRAP_H, 0.0), (4.0, 0.0, 0.0), APRON)
b.crest_ns("tuft_w", 0.5, -TRAP_H, 0.0, APRON_FLOWERS[0])
b.crest_ns("tuft_e", 2.5, -TRAP_H, 0.0, APRON_FLOWERS[1])

# =================================================================
# 7. 步道两端收边: 外沿绿篱直立三角 x2 + 南沿角花直立三角 x2
# =================================================================
b.crest_ew("hedge_w", -1.0, 0, 0.0, HEDGE)
b.crest_ew("hedge_e", 5.0, 0, 0.0, HEDGE)
b.crest_ns("corner_w", -1.0, 0.0, 0.0, CORNER_FLOWERS[0])
b.crest_ns("corner_e", 4.0, 0.0, 0.0, CORNER_FLOWERS[1])
...
b.step(
    "铺步道包边: 两片绿色梯形平铺在步道南沿当扇贝形包边, 三片绿色"
    "三角补花砖填平斜边豁口, 檐前再种两丛三角花。",
    ["apron_w", "apron_e", "scallop_w", "scallop_c", "scallop_e",
     "tuft_w", "tuft_e"],
    highlight=["walk_w", "walk_e"],
    tip="斜披、直立、平挑、平铺 —— 同一种梯形摆出四种姿态; 补花砖让扇贝连成整条饰带。",
)
b.step(
    "延伸步道收边: 两片灰色方板接住步道东西两端, 外沿各立一片绿篱, "
    "南沿再种两株角花 —— 梯形雨檐花架墙落成!",
    ["walk_ext_w", "walk_ext_e", "hedge_w", "hedge_e",
     "corner_w", "corner_e"],
    highlight=["walk_w", "walk_e"],
    tip="延伸板一边吸长石板短边、一边吸碎石南沿 —— 赏花步道两头都有花迎客。",
)
...
b.finalize(
    ...
    description=(
        ...
        "步道南沿再平铺两片扇贝形包边, 三片三角补花砖把扇贝连成整条"
        "饰带; 花圃带五株骑缝三角花, 檐前两丛花收边, 步道两端延伸方"
        "板配绿篱角花迎客。梯形下底 (长 2) 与长方形横边恰好等长, 是"
        "整面墙整边吸合的几何前提 —— 与旧版连廊密铺条带的动线完全不同。"
    ),
    difficulty=2,
