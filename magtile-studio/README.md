...
- **标准磁力片形状库**: 共 13 种, 分核心/扩展两层 (见 `docs/TILE_CATALOG.md` 与 `docs/TILE_SET.md`) —— 核心 5 种 (实物基础套装, 依据实物照片确认): 正方形、等边三角形、直角三角形、等腰三角形 (瘦高, 底 1 高 2)、长方形; 扩展 8 种: 菱形、梯形、六边形、扇形、大正方形、窗格方、门框方、车轮底座。几何数据由 `data/tile_catalog.json` 驱动, 可扩展非标配件。
...
├── data/
│   ├── tile_catalog.json   # 13 种标准磁力片的几何与磁力边定义 (核心 5 + 扩展 8)
